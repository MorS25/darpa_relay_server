...
cbor::cbor (cbor::binary value) : m_type (cbor::TYPE_BINARY), m_binary (std::move(value)) {
}
cbor::cbor (const cbor::binary &value) : m_type (cbor::TYPE_BINARY), m_binary (value) {
}
cbor::cbor (cbor::string value) : m_type (cbor::TYPE_STRING), m_string (std::move(value)) {
}
...
cbor::cbor (cbor::array value) : m_type (cbor::TYPE_ARRAY), m_array (std::move(value)) {
}
cbor::cbor (const cbor::array &value) : m_type (cbor::TYPE_ARRAY), m_array (value) {
}
cbor::cbor (cbor::map value) : m_type (cbor::TYPE_MAP), m_map (std::move(value)) {
}
...
uint64_t cbor::to_unsigned () const {
  ...
  case cbor::TYPE_SIMPLE:
    if (this->m_value == cbor::SIMPLE_TRUE)
      return 1;
    else if (this->m_value == cbor::SIMPLE_FALSE)
      return 0;
    else
      throw std::runtime_error("type cannot convert to unsigned, it is " + std::string(this->type_name()));
...
int64_t cbor::to_signed () const {
	...
	case cbor::TYPE_FLOAT:
		return this->m_float;
	default:
    throw std::runtime_error("type cannot convert to signed, it is " + std::string(this->type_name()));
	}
}
const cbor::binary& cbor::to_binary () const {
	switch (this->m_type) {
	...
	default:
    throw std::runtime_error("type cannot convert to binary, it is " + std::string(this->type_name()));
	}
}
const cbor::string& cbor::to_string () const {
	switch (this->m_type) {
	...
	default:
    throw std::runtime_error("type cannot convert to string, it is " + std::string(this->type_name()));
	}
}
const cbor::array& cbor::to_array () const {
	switch (this->m_type) {
	...
	default:
    throw std::runtime_error("type cannot convert to array, it is " + std::string(this->type_name()));
	}
}
const cbor::map& cbor::to_map () const {
	switch (this->m_type) {
	case cbor::TYPE_MAP:
		return this->m_map;
	case cbor::TYPE_TAGGED:
	...
	default:
    throw std::runtime_error("type cannot convert to map, it is " + std::string(this->type_name()));
	}
}
cbor::simple cbor::to_simple () const {
	...
	default:
    throw std::runtime_error("type cannot convert to simple, it is " + std::string(this->type_name()));
	}
}
bool cbor::to_bool () const {
  ...
  case cbor::TYPE_NEGATIVE:
	...
	default:
    throw std::runtime_error("type cannot convert to bool, it is " + std::string(this->type_name()));
	}
}
double cbor::to_float () const {
	...
	case cbor::TYPE_FLOAT:
		return this->m_float;
	default:
    throw std::runtime_error("type cannot convert to float, it is " + std::string(this->type_name()));
	}
}
cbor::operator unsigned char () const {
...
cbor::operator const cbor::binary& () const {
	return this->to_binary ();
}
cbor::operator const cbor::string& () const {
	return this->to_string ();
}
cbor::operator const cbor::array& () const {
	return this->to_array ();
}
cbor::operator const cbor::map& () const {
	return this->to_map ();
}
...
const char * cbor::type_name() const {
  switch (this->m_type)
  {
    case cbor::TYPE_UNSIGNED:
      return "unsigned";
...
void read_uint (std::istream &in, int &major, int &minor, uint64_t &value) {
	...
	case 24:
		value |= static_cast<uint64_t>(in.get());
		break;
	default:
		value = minor;
		break;
...
std::ostream& hex_dump(std::ostream& os, void *buffer, size_t bufsize)
{
  auto f = os.flags();
  auto c = os.fill();
  os << std::hex;
  ...
  for (size_t i = 0; i < bufsize; ++i)
  {
    if (space)
      os << ' ';
    os << std::setw(2) << std::right << static_cast<unsigned>(reinterpret_cast<unsigned char*>(buffer)[i]);
    if (!space)
      space = true;
  }
  os.fill(c);
  ...
  cbor item;
	...
	case 7:
		...
		case 27:
			item.m_type = cbor::TYPE_FLOAT;
      item.m_value = value;
      // hex_dump(std::cout, reinterpret_cast<void*>(&item.m_value), 8) << " = " << item.m_float << std::endl;
			break;
		default:
...
cbor cbor::decode (const std::string &in) {
  std::istringstream buf1 (in);
  cbor buf2;
  if (buf2.read (buf1) && buf1.peek () == EOF) {
    return buf2;
  }
